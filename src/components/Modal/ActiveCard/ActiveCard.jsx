...
import {
  ...
  updateCurrentAciveCard
} from '~/redux/activeCard/activeCardSlice'
import { updateCardAPI } from '~/apis'
import { updateCard } from '~/redux/activeBoard/activeBoardSlice'
...
function ActiveCard() {
  ...
  const apiUpdate = async (updateData) => {
    const updatedCard = await updateCardAPI(activeCard._id, updateData)
    dispatch(updateCurrentAciveCard(updatedCard))
    dispatch(updateCard(updatedCard))
    return updatedCard
  }

  const onUpdateCardTitle = (newTitle) => {
    apiUpdate({ title: newTitle.trim() })
  }
  ...
  const onUploadCardCover = (event) => {
    ...
    // Gọi API...
    toast.promise(
      apiUpdate(reqData).finally(() => (event.target.value = '')),
      { pending: 'Updating...' }
    )
  }

  const onUpdateDescription = (newDescription) => {
    apiUpdate({ description: newDescription })
  }

  const addCardComment = async (comment) => {
    await apiUpdate({ comment })
  }

  const onUpdateCardMembers = (memberInfor) => {
    apiUpdate({ memberInfor })
  }
          ...
          <Grid xs={12} sm={9}>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <CardDescriptionMdEditor
                ...
                handleUpdateCardDesc={onUpdateDescription}
              />
            </Box>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <CardActivitySection cardComments={activeCard?.comments} addCardComment={addCardComment} />
            </Box>
          </Grid>
          ...
          <Grid xs={12} sm={3}>
            ...
            <Stack direction="column" spacing={1}>
              ...
              {!activeCard?.memberIds?.includes(currentUser._id) && (
                <SidebarItem
                  onClick={() => onUpdateCardMembers({ userId: currentUser._id, action: MEMBER_CARD_ACTIONS.ADD })}
                  className="active"
                >
