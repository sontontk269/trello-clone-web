...
function CardActivitySection({ cardComments = [], addCardComment }) {
  ...
  const handleAddCardComment = (event) => {
    ...
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      //goi component cha
      addCardComment(commentToAdd).then(() => {
        event.target.value = ''
      })
    }
  }
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
        <Avatar sx={{ width: 36, height: 36, cursor: 'pointer' }} alt="" src={currentUser?.avatar} />
        <TextField
          fullWidth
